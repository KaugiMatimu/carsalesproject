...
<html lang="en">
  ...
  <body>
    ...
    @if(session()->has('message'))
            ...
            @endif
        <!-- Caption -->
        <h2 class="mb-4 text-center text-white">Add Products</h2>
        
        <!-- Form -->
        <form class="needs-validation p-4 bg-dark rounded" action="{{url('/add_products')}}" method="POST" novalidate style="width: 100%; max-width: 500px;" enctype="multipart/form-data">
        @csrf    
        <div class="form-row mb-3">
                <div class="col-md-12">
                    <label for="productName" class="text-white">Product Name</label>
                    <input type="text" class="form-control bg-light text-dark border-light" name="title" id="productName" placeholder="Write Product Name" required>
                </div>
            </div>
            <div class="form-row mb-3">
                <div class="col-md-12">
                    <label for="productDescription" class="text-white">Product Description</label>
                    <input type="text" class="form-control bg-light text-dark border-light" name="description" id="productDescription" placeholder="Write Product Description" required>
                </div>
            </div>
            <div class="form-row mb-3">
                <div class="col-md-12">
                    <label for="productPrice" class="text-white">Product Price</label>
                    <input type="number" class="form-control bg-light text-dark border-light" name="price" min="0" id="productPrice" placeholder="Write Product Price" required>
                </div>
            </div>
            <div class="form-row mb-3">
                <div class="col-md-12">
                    <label for="productQuantity" class="text-white">Quantity</label>
                    <input type="number" class="form-control bg-light text-dark border-light" name="quantity" id="productQuantity" placeholder="Enter Product Quantity" required>
                </div>
            </div>
            <div class="form-row mb-3">
                <div class="col-md-12">
